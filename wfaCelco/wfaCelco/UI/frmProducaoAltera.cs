...
{
    public partial class frmProducaoAltera : Form
    {
        ...
        RecursosBLL REC = new RecursosBLL();
        int cod = 0;
        string COD;

        public frmProducaoAltera(string codigo)
        {
            InitializeComponent();
            ...
            txtQtde.Value = Convert.ToInt32(REC.Informacao("PRODUCAO_QTD", "PRODUCAO", " WHERE PRODUCAO_ID = " + codigo));
            dtInicio.Value = Convert.ToDateTime(REC.Informacao("PRODUCAO_DATA_INICIO", "PRODUCAO", " WHERE PRODUCAO_ID = " + codigo));
            dtFim.Value = Convert.ToDateTime(REC.Informacao("PRODUCAO_DATA_FIM", "PRODUCAO", " WHERE PRODUCAO_ID = " + codigo));
            txtObs.Text = REC.Informacao("PRODUCAO_OBS", "PRODUCAO", " WHERE PRODUCAO_ID = " + codigo);

            REC.ItemRecurso(dgvAux, codigo);

            for (int linha = 0; linha < dgvAux.Rows.Count; linha++)
            {
                string ID = dgvAux.Rows[linha].Cells[0].Value.ToString();
                string IDREC = dgvAux.Rows[linha].Cells[1].Value.ToString();
                string NOME = REC.Informacao("RECURSO_NOME", "RECURSOS ", "WHERE RECURSO_ID = " + IDREC);
                string QTD = dgvAux.Rows[linha].Cells[2].Value.ToString();
                dgvItem.Rows.Add(ID, NOME, QTD);
            }
      ...
      /*========================
        ...
        public void salva()
        {
            ...
            {                
                ...
                dto.Producao_qtd = Int32.Parse(this.txtQtde.Text);
